 ...
 * @returns
 */
export async function genStyleInjectionCode(styles: Array<SFCStyleBlock & { css?: string }>, parentId: string) {
  const styleCodes: any[] = [];
...
export function stylesDeleteHandler() {
  ...
  const oldStyle: any = document.getElementById(ssrIdKey);
  if (oldStyle) {
...
export const insertCustomCssToHead = (cssCode: string, id: string) => {
  const head = document.head || document.getElementsByTagName('head')[0];
  const oldStyle: any = document.getElementById(id);
  if (oldStyle) {
    head.removeChild(oldStyle);
  }
...
export async function styleLoader(
  ...
  let result;
  try {
    const implementation = nameSource;
    switch (name) {
      case 'less':
    ...
    console.log('%c Style解析错误:', 'color: #FFFFFF; background: #f5222d; font-size: 13px;', error);
    return;
  }
  return result;
}
